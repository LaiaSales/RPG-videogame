...
// 2) Parseamos la ROOT ARRAY (colisiones) en sub-arrays (FILAS de 190 tiles (width del mapa)).
for (let i = 0; i < colisiones.length; i += 190) {
    colisionesMapa.push(colisiones.slice(i, 190 + i)) //Añadimos nuevos items a la array colisionesMapa. En vez de la expresión estática colisiones.slice(0, 190) la hacemos dinámica para que en cada iteration del loop empiece con la siguiente fila.
}

// 4) Como tendremos muchos límites en el mapa creamos "const limites" que los almacena.
const limites = []
const desviacion = {
    x: -4265, 
    y: -1830, 
...
colisionesMapa.forEach((fila, i) => { //el índex (i) de la fila por la que pasa el loop ahora
    fila.forEach((simbolo, j) => { // el índex (j) del símbolo de la fila por la que pasa el loop ahora
        if (simbolo === 17343) // solo queremos push un Limite si el símbolo por el que pasa el loop es 17343 (numero de tile colisión)
        limites.push(
            new Limite({ //usas la class Limite
                position: { 
...
const jugador = new Sprite({
    position: {
        x: canvas.width / 2 - 148 / 4 / 2, //Centrado en eje x //No usamos "ImagenJugador.width" porque la imagen no se cargaría a tiempo ya que const jugador se llama en cuento ejecutamos el file. En su lugar, usamos la altura (208px) y ancho (78px) de la imagen 
...
function animacion() {
    ...
    if(teclas.w.pressed && ultimaTecla ==='w') {
        ...
        movibles.forEach((movible) => { //por cada movible dentro del array movibles se le añade 4 a medida que pasa el tiempo
            movible.position.y += 4 // movible.position.Y += 4 es lo mismo que movible.position.Y = movible.position.Y + 4
        }) 
